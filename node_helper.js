...
const fliclib = require( __dirname + "/lib/fliclib-linux-hci/clientlib/nodejs/fliclibNodeJs.js")
...
module.exports = NodeHelper.create({
  ...
  listenToButton: function(bdAddr) {
    ...
    cc.on("buttonUpOrDown", function(clickType, wasQueued, timeDiff) {
      parent.sendSocketNotification("buttonUpOrDown",
        {bdAddr: bdAddr, clickType: clickType, wasQueued: wasQueued, timeDiff: timeDiff})
    })
    cc.on("connectionStatusChanged", function(connectionStatus, disconnectReason) {
      parent.sendSocketNotification("connectionStatusChanged",
        {bdAddr: bdAddr, connectionStatus: connectionStatus, disconnectReason: disconnectReason})
    })
  },
  ...
  startFlicd: function() {
    ...
    const flicd = spawn(flicdDir + "/flicd",['-f', __dirname + "/flic.sqlite3"])
    ...
    return new Promise((resolve) => {
      flicd.stderr.on('data', (data) => {
        ...
        if (data.toString().endsWith("Initialization of Bluetooth controller done!\n")){
